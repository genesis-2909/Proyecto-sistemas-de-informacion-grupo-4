...
class _DetalleRutaViewState extends State<DetalleRutaView> {
  ...
  void _ejecutarReserva() async {
    if (_fechasSeleccionadas == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Por favor, seleccione el rango de fechas para su estadía.')),
      );
      return;
    }
  ...
  void _abrirPaypalSandbox() {
    showModalBottomSheet(
      ...
      builder: (context) => _PaypalSandboxWidget(
        ...
        onPagoExitoso: () async {
          setState(() { _estadoFlujo = 'Procesando Pago'; });
          
          bool exito = await _reservaCtrl.procesarPagoReal(_idDocumentoReservaCreada);
  ...
  void _agregarResena() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('publicaciones').doc(widget.sitio.id).collection('resenas').add({
      ...
      setState(() { _calificacionNueva = 5; });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('¡Reseña añadida!')));
      }
    } catch (e) {
            ...
            children: [
              ...
              Container(
                ...
                child: const Column(
                  ...
                  children: [
                    Icon(Icons.map_outlined, size: 45, color: Color(0xFF1B5E20)),
                    SizedBox(height: 8),
                    Text('Detalle de la Ruta Turística', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
              ...
              Row(
                ...
                children: [
                  ...
                  Text('\$${widget.sitio.costoMaximo.toInt()} / Noche', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.green)),
                ],
              ),
              const SizedBox(height: 15),
              Text(widget.sitio.descripcion, style: const TextStyle(fontSize: 15)),
              const Divider(height: 40),
              
              // SECCIÓN DE RESERVAS / CONFIGURACIÓN DE VIAJE
              if (_estadoFlujo == 'Disponible') ...[
                const Text('Fechas de Estadía:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ...
                const SizedBox(height: 15),
                ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1B5E20), minimumSize: const Size(double.infinity, 50)),
                  onPressed: _ejecutarReserva,
                  child: const Text('Solicitar Reserva', style: TextStyle(color: Colors.white, fontSize: 16)),
                )
              ],
              ...
              if (_estadoFlujo == 'Solicitado') ...[
                Container(
                  ...
                  child: const Text('Estado: SOLICITADO.', style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),
                ),
                const SizedBox(height: 20),
                ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF003087), minimumSize: const Size(double.infinity, 52)),
                  onPressed: _abrirPaypalSandbox, 
                  icon: const Icon(Icons.payment, color: Colors.white),
                  label: const Text('Pagar con PayPal', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                )
              ],
              ...
              if (_estadoFlujo == 'Procesando Pago') ...[
                const Center(
                  child: Padding(
                    ...
                    child: Column(children: [CircularProgressIndicator(color: Color(0xFF1B5E20)), SizedBox(height: 10), Text('Procesando transacción...')]),
                  ),
                )
              ],
              ...
              if (_estadoFlujo == 'Pagado') ...[
                Container(
                  width: double.infinity, padding: const EdgeInsets.all(16), color: Colors.green[50],
                  child: const Column(children: [
                    Icon(Icons.check_circle, color: Colors.green, size: 50),
                    SizedBox(height: 10),
                    Text('¡PAGO APROBADO!', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                  ]),
                ),
              ],
              ...
              if (_estadoFlujo == 'Ocupado') ...[
                Container(
                  width: double.infinity, padding: const EdgeInsets.all(16), color: Colors.red[50],
                  child: const Text('Ruta turística no disponible en este momento.', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                ),
              ],
              ...
              Row(
                children: [
                  ...
                  DropdownButton<int>(
                    ...
                    onChanged: (v) => setState(() { _calificacionNueva = v!; }),
                  ),
                ],
              ),
              TextField(
                controller: _resenaController,
                decoration: const InputDecoration(hintText: 'Añade un comentario', border: OutlineInputBorder()),
              ),
              const SizedBox(height: 10),
              Align(
              ...
              StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                    return const Text('Sin reseñas de costos reales.', style: TextStyle(fontStyle: FontStyle.italic, color: Colors.grey));
                  }
...
class _PaypalSandboxWidget extends StatefulWidget {
  ...
  const _PaypalSandboxWidget({required this.monto, required this.onPagoExitoso});
...
class _PaypalSandboxWidgetState extends State<_PaypalSandboxWidget> {
  ...
  final _emailSandboxController = TextEditingController(text: 'unimet-buyer@sandbox.paypal.com');

  void _procesarTransaccionSandbox() async {
    setState(() { _cargandoPasarela = true; });
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('PayPal', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, fontStyle: FontStyle.italic, color: Color(0xFF003087))),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: Colors.yellow[700], borderRadius: BorderRadius.circular(5)),
                child: const Text('SANDBOX', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.black)),
              ),
            ],
          ),
          const Divider(height: 30),
          if (!_cargandoPasarela) ...[
            Text('Resumen del Pago Técnico:', style: TextStyle(color: Colors.grey[600], fontSize: 14)),
            Text('\$${widget.monto.toStringAsFixed(2)} USD', style: const TextStyle(fontSize: 30, fontWeight: FontWeight.bold, color: Colors.black)),
            const SizedBox(height: 20),
            TextField(
            ...
            ElevatedButton(
              ...
              child: const Text('Completar Compra ', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 16)),
            ),
            const SizedBox(height: 15),
          ] else ...[
            const Padding(
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(height: 10),
                  Text('Autorizando Fondos...', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
                ],
              ),
            )
          ]
